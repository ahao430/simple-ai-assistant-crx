...
- GIF 生成支持可调帧延迟（50-500ms）。
  - 生成控制区新增"帧延迟"滑块。
  - GIF 生成后可调整速度并重新合成。
  - "应用新速度"按钮快速重新生成 GIF。
- 历史 GIF 支持速度调整。
  - 数据库升级到 v3，新增 `gifFrames` 存储保存原始帧。
  - 历史详情页新增速度调整滑块。
  - 调整后的 GIF 自动保存回数据库。
- GIF 生成模型选择器增加标签提示（文本模型、生图模型）。

### Changed

- 优化 GIF 提示词生成。
  - 上传参考图时明确要求使用参考图中的形象和风格。
  - 强调"seamless grid with no gaps or borders"避免切割抖动。
  - 明确要求"no frame numbers or text labels"。
- 修复参考图功能，上传参考图后正确调用 `/images/edits` 接口。
